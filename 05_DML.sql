...
VALUES('900', '장채현', '901230-1234567', 'jang_ch@kh.or.kr',
...
-- 선택안된 컬럼은 값이 NULL이 들어감(DEFAULT 존재시 DEFAULT값으로 삽입됨)
-- 실무에서는 이렇게 많이 사용. 조금 더 빠르다
...
-- 서브쿼리(SELECT) 결과를 ENP_01 테이블에 INSERT
--> SELECT 조회 결과의 데이터타입, 컬럼 갯수가 INSERT하려는 테이블의 컬럼과 일치해야한다
...
DEPT_ID = 'D0',
DEPT_TITLE = '전략기획2팀' -- WHERE절 직전 컬럼 작성후 콤마 X
WHERE DEPT_ID = 'D9'
...
SELECT SALARY FROM EMPLOYEE2 WHERE EMP_NAME = '유재식'; -- 유재식 급여

SELECT BONUS FROM EMPLOYEE2 WHERE EMP_NAME = '유재식'; -- 유재식 보너스
...
SELECT * FROM EMP_M02; -- 기존 4명 + 신규1명
...
ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID) -- 이 컬럼을 사용해 중복판별
WHEN MATCHED THEN -- 같을때(컬럼의 내용이 중복될때)
UPDATE SET -- UPDATE 구문 사용
...
WHEN NOT MATCHED THEN -- 같지 않을때
INSERT VALUES -- EMP_M01 테이블에 새로운 행 추가
