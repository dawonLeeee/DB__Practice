/* VIEW
	...
	** VIEW 사용 목적 **
	* 복잡한 SELECT문을 쉽게 재사용하기 위해서 사용
	* 테이블의 진짜 모습을 담을 수 있어 보안상 유리함
	* 
	** VIEW 사용시 주의사항 **
	* 가상의 테이블(실제 테이블X) -> ALTER구문 사용불가
	* VIEW를 이용한 DML(INSERT/UPDATE/DELETE)가 가능한 경우도 있지만 많은 제약이 따르기 때문에
...
JOIN JOB USING(JOB_CODE); -- 안됨
-- ORA-01031: 권한이 불충분합니다
--> 해결 : 
--		1)SYS 관리자 계정 접속
--		2)ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;( ##없앰)
--		3)GRANT CREATE VIEW TO kh_ldw;
...
INSERT INTO V_DCOPY2 VALUES('D0', 'L3'); -- 안됨
-- ORA-01400: NULL을 ("KH_LDW"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

--> VIEW를 이용한 INSERT시 VIEW가 아닌 원본 테이블에 삽입이 진행된다. 
--> VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에는 INSERT시 NULL이 삽입됨('D0', NULL, 'L3')
