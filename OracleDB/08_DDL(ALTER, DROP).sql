...
ALTER TABLE DEPARTMENT_COPY ADD CONSTRAINT ADD_UNIQUE UNIQUE(DEPT_TITLE);


-- DEPT_COPY의 DEPT_TITLE컬럼에 UNIQUE 삭제
ALTER TABLE DEPARTMENT_COPY DROP CONSTRAINT ADD_UNIQUE;

-- DEPT_COPY의 DEPT_TITLE컬럼에 NOT NULL 제약조건 추가/삭제
ALTER TABLE DEPARTMENT_COPY 
ADD CONSTRAINT DEPT_COPY_TITLE_NN NOT NULL(DEPT_TITLE); --오류남
--> NOT NULL 제약조건은 새로운 조건을 추가하는 것이 아니라
...
-- ALTER TABLE 테이블명 DROP COLUMN 컬럼명; -- 같은 의미
...
SELECT * FROM DEPARTMENT_COPY dc ;

-- 컬럼 추가
ALTER TABLE DEPARTMENT_COPY ADD(CNAME VARCHAR2(30));
ALTER TABLE DEPARTMENT_COPY ADD(LNAME VARCHAR2(30) DEFAULT '한국');
--> 컬럼이 생성되면서 DEFAULT값이 자동 삽입됨


-- D10 개발1팀 추가
INSERT INTO DEPARTMENT_COPY VALUES('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);-- 오류
-- "KH_LDW"."DEPARTMENT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- 컬럼 수정 후 재실행

-- 컬럼 수정
ALTER TABLE DEPARTMENT_COPY MODIFY DEPT_ID VARCHAR2(3);

-- DEFAULT값 수정(LNAME의 기본값을 'KOREA'로 수정)
ALTER TABLE DEPARTMENT_COPY MODIFY LNAME DEFAULT 'KOREA';
--> 기본값을 변경했다고 해서 기존 데이터가 변하지는 않음

--LNAME '한국' -> 'KOREA'변경
UPDATE DEPARTMENT_COPY SET LNAME = DEFAULT
WHERE LNAME = '한국';
...
ALTER TABLE DEPARTMENT_COPY DROP COLUMN CNAME;
ALTER TABLE DEPARTMENT_COPY DROP(LOCATION_ID);
ALTER TABLE DEPARTMENT_COPY DROP(DEPT_TITLE);
ALTER TABLE DEPARTMENT_COPY DROP(DEPT_ID);-- 테이블에 모든 열들을 삭제할 수 없습니다
...
--DEPARTMENT테이블 복사해서 DEPT_COPY 생성
CREATE TABLE DEPT_COPY
AS SELECT * FROM DEPARTMENT d ;
--> 제약조건이 복사가 안됨(컬럼명/데이터타입/NOT NULL여부만 복사 됨)
--> PRIMARY KEY 없음

SELECT * FROM DEPT_COPY;

-- DEPT_COPY에 PK추가(DEPT_ID컬럼에 D_COPY_PK)
...
CREATE TABLE TB2(
	...
	TB2_COL NUMBER REFERENCES TB1 -- 1, 2, 3, NULL 삽입가능
);
SELECT * FROM TB2;
...
--3) DROP TABLE 삭제옵션 CASCADE CONSTRAINTS 사용(2번 해결방법을 1줄로 줄임)
 ...
 * --> DDL은 수행시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT을 시킴
 * --> DDL은 수행시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT을 시킴
 **/
