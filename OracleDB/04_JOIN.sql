...
/* 
 * (참고)
 * - JOIN은 서로 다른 테이블의 행을 하나씩 이어붙이기 때문에 
 * 		시간이 오래 걸리는 단점이 있다
 * 
 * 
 * 
...
-- 오라클
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT -- 여기 테이블들에서 조회할거야
WHERE DEPT_CODE = DEPT_ID; 
-- 단, EMPLOYEE테이블의 DEPT_CODE와 DEPARTMENT테이블의 DEPT_ID가 같은거만 조회할거야
...
-- 오라클 구문
-- 포함시키고자 하는 반대쪽 테이블 컬럼에 (+) 기호를 작성해야 한다
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID;
...
-- SAL_LEVEL이 EMPLOYEE와 SAL_GRADE테이블에 둘다 있으므로 "SAL_GRADE.SAL_LEVEL" 표기
FROM EMPLOYEE
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
-- 팁: 같은 테이블이 2개 있다고 생각하고 JOIN 진행하면 됨
...
-- 사원 이름, 부서명, 지역명 조회 
-- > EMPLOYEE, DEPARTMENT, LOCATION테이블에 존재
-- ANSI 표준
...
AND LOCATION_ID = LOCAL_CODE; -- 조건이 여러개일땐 논리연산자 사용
...
-- 오라클 전용
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, E.JOB_CODE, LOCAL_NAME, SALARY
...
-- LEFT JOIN 꼭 사용해야 한다(왜? NULL값 포함시키기 위해)
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE) --여기서 부서가 없는사람들 걸러줌
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE DEPT_CODE IS NOT NULL; -- 이거 안써도 된다
 ...
 * 6. ORDER BY
 */
--> FROM(+JOIN)절에서 SAL_GRADE테이블을 명시하면 WHERE절에서 SAL_GRADE테이블을 사용할 수 있다
--> JOIN한 테이블을 조건에 사용할 수 있다
SELECT DISTINCT EMP_NAME, JOB_NAME, SALARY 급여, SALARY * (1 + NVL(BONUS, 0)) * 12 "연봉"
FROM EMPLOYEE E1
NATURAL JOIN JOB
...
-- NATIONAL은 예약어이지만 JOIN문에 쓰여서 테이블명으로 읽힘. 거슬리면 쌍따옴표"" 쓰기
...
WHERE E1.EMP_NAME <> E2.EMP_NAME  -- 나의 동료는 나다? XX -> 중복 제거
ORDER BY 1;
