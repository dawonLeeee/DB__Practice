...
--------------------------------------------------------------------------------------------------------------------
-- DQL(Data Query Language) : 데이터 질의 언어(데이터 조회)
-- DML(Data Manipulation Language) : 데이터 조작 언어, 테이블에 데이터를 삽입, 수정, 삭제
...
/* 자료형
    ...
    DATE : 날짜 타입
    BLOB : 대용량 이진 데이터 (4GB) : 대용량 이진 데이터 (4GB)
    	→글이 아니어도 저장 가능(동영상, 실행파일)/지양하는 추세(속도 문제)
    CLOB : 대용량 문자 데이터 (4GB)
*/
...
-- SQL작성법 : 대문자 작성 권장, 연결된 단어 사이에는 "_" (언더바) 사용
-- 문자 인코딩 UTF-8 : 영어/숫자는 1BYTE, 한글/특수문자 등은 3BYTE 취급
-----> MEMBER_NAME 작성시 문자인코딩 고려해서 길이제한을 둬야 한다
...
-- INSERT나 UPDATE 시 컬럼 값으로 DEFAULT를 작성하면 테이블 생성시 해당 컬럼에 지정된 DEFAULT값으로 삽입됨
INSERT INTO "MEMBER" VALUES('MEM01', '123ABC', '홍길동', '991123-1234567', DEFAULT);
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES('MEM02', 'QWER1234', '김영희', '971022-1247853', DEFAULT);


-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES('MEM03', 'DF2343', '박철수', '900121-8527413', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
--INSERT INTO 테이블명(컬럼명1, 컬럼명2, 컬럼명3) VALUES(값1, 값2, 값3);
INSERT INTO "MEMBER"(MEMBER_ID, MEMBER_PWD, MEMBER_NAME) VALUES('MEM04', '1QS2321', '이지연');
...
-- '2022-09-13 17:33:12'
INSERT INTO "MEMBER" VALUES('MEM05', 'PASS05', '김길동', '930210-2245678', 
TO_DATE('2022-09-13 17:33:12', 'YYYY-MM-DD HH24:MI:SS'));
...
INSERT INTO MEMBER2 VALUES('MEM02', 'PASS02', '고길순', 01057060665);
-- NUMBER타입 컬럼에 데이터 삽입시 제일 앞에 0이 있으면 이를 자동으로 제거함.
----> 전화번호, 주민번호 처럼 숫자로만 되어있는 데이터이지만 0으로 시작할 가능성이 있으면 CHAR, VARCHAR2 같은 문자형을 사용
...
-- EX)아이디, 비밀번호, 게시글에서 제목, 내용, 작성자, 날짜 등 ( 대부분의 컬럼이 NOT NULL이다)
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL, -- 사용자 번호(모든 사용자는 사용자번호가 있어야 한다) / 컬럼레벨 제약조건
...
-- 테이블에 대한 식별자(IDENTIFIER) 역할을 함(→전화번호, 학번, 주민번호…)
...
--> 기본키 중복으로 오류 // 무결성 제약 조건(KH_LDW.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류 // NULL을 ("KH_LDW"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- (참조할 컬럼을 명시하지 않으면 참조할 테이블의 PK값 자동 참조)
...
-- FOREIGN KEY라는 이름은 테이블 레벨에서만 사용
...
-- CHECK 제약 조건은 범위로도 설정 가능.
 ...
 -- CHECK 제약조건은 FK 제약조건의 하위호환이다!
--> 둘 다 컬럼의 값을 지정된 형태로만 제한을 하지만 
--	CHECK는 작성 후 수정이 번거롭지만 FK는 부모테이블에 행을 추가, 수정, 삭제하는게 간편함
 ----------------------------------------------------------------------------------------------------------------
...
--> SUBQUERY의 조회결과(RESULTSET)의 모양대로 테이블이 생성됨
...
--> 서브쿼리로 테이블 생성 시
-- 테이블의 형태(컬럼명, 데이터 타입, NOT NULL)만 복사되고
-- 제약조건, 코멘트는 복사되지 않기 때문에 별도 추가 작업이 필요하다!

-- CF) DDL : CREATE, ALTER, DROP(테이블의 데이터를 생성하고 정의하는 언어)
