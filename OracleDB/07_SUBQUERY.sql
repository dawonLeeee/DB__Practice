...
/*  서브쿼리 유형
    ...
    - 단일행(단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때 (조회결과가 1행1열)
...
WHERE SALARY > (SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE e2 )
ORDER BY JOB_CODE ; -- SELECT절에 없어도 FROM, JOIN테이블에 존재하는 컬럼이라면 정렬 가능!
...
-- 사수에 해당하는 직원에 대해 조회 (사수 = MANAGER_ID 컬럼에 작성된 사람)
...
-- 선택함수 사용
--> DECODE(컬럼명, 값1, 1인경우, 값2, 2인경우, ..., 일치하지 않은 경우);
...
NATURAL JOIN JOB 
WHERE JOB_NAME = '대리'
AND SALARY > ANY(SELECT SALARY -- 가장 작은거보다 큰 경우만 조회
FROM EMPLOYEE e 
NATURAL JOIN JOB j 
WHERE JOB_NAME = '과장');
...
WHERE (DEPT_CODE, JOB_CODE) IN (
	SELECT DEPT_CODE, JOB_CODE FROM EMPLOYEE e2 
	WHERE EMP_NAME = '노옹철'
)
AND EMP_NAME <> '노옹철';
...
WHERE (DEPT_CODE, MANAGER_ID) IN (
	SELECT DEPT_CODE, MANAGER_ID FROM EMPLOYEE e2 
...
-- 1) MAIN QUERY의 1행의 DEPT_CODE를 SUB QUERY에 대입
-- 2) SUB QUERY를 수행
-- 3) SUB의 결과를 이용해 MAIN을 수행
...
SELECT EMP_NAME, JOB_CODE, (SELECT MAX(SALARY) FROM EMPLOYEE e2 )- SALARY
FROM EMPLOYEE e;
...
SELECT EMP_NAME, JOB_CODE, SALARY,
	(SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE E2 WHERE E1.JOB_CODE = E2.JOB_CODE)
FROM EMPLOYEE E1 ;
...
--1) 이름, 급여를 급여 내림차순으로 조회한 결과를 인라인뷰로 사용
	--> FROM절에서 작서오디기 때문에 해석1순위
--2) 메인쿼리 조회시 ROWNUM을 5이하까지만 조회

SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME, SALARY FROM EMPLOYEE e
	ORDER BY SALARY DESC 
)
WHERE ROWNUM <= 5;
...
SELECT DEPT_CODE, DEPT_TITLE, CEIL(AVG(SALARY)) 평균급여 FROM EMPLOYEE e 
